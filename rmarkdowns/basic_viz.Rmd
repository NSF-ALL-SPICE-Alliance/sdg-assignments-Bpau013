...
```{r}
ggplot(data=sdr_data, aes(x=goal_5_score, y=goal_1_score, color=goal_2_score)) + geom_point()
```
Some countries are real high in the goal_1_score category, but low in both goal_2 and goal_5_scores. A vast majority of countries have scored high in goal_1_score.
### Bar Chart
...
```
```{r}
ggplot(MENA_sdr_data, aes(x=goal_2_score, y=country)) + geom_bar(stat = "identity")
```
...
```{r}
ggplot(sdr_data, aes(x=goal_2_score, fill=regions_used_for_the_sdr)) + geom_histogram(bins=33, color="black")
```
Majority of the countries fall with the 50-70 range when based on the goal_2_score. But there are a few countries that have scored higher than 80.