...
```{r}
goal_3_scatterplot <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr, fill = regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal() +
  scale_fill_viridis_d() +
  labs(title = "Distribution of multiple SDG scores",
       x = "SDG 3 Score", 
       y = "SDG 4 Score", 
       fill = "Region")
...
mytext <- paste(
    "Country: ", sdr_data_world_joined$country,"<br/>", 
    "Goal 5 Score: ", round(sdr_data_world_joined$goal_2_score, 2), 
    sep="") %>%
  lapply(htmltools::HTML)
...
leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("YlGn", goal_5_score)(goal_5_score), label = mytext)
```
