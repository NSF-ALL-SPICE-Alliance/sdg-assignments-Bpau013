...
```
```{r}
importance_dfi <- as.data.frame(rf_water$importance)
                               
```
```{r}
importance_dfi_top_10 <- importance_df %>%
  rownames_to_column(var = "variable") %>%
  slice_max(n = 10, order_by = `%IncMSE`)
```
```{r}
ggplot(importance_dfi_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  geom_bar(stat = "identity", fill = "steelblue", color = "black") +
  theme_minimal() +
  labs(title = "Most Important Variables in Predicting Water Accessibility",
       subtitle = "Top 10",
       y = "SDG Indicator",
       x = "Feature Importance (% Increase in Mean Squared Error)")
```
...
```
```{r}
pd <- pdp::partial(rf_water, pred.var = c("normalized_score_sdg7_elecac", "normalized_score_sdg6_sanita"))
# Default PDP
plotPartial(pd)
```
